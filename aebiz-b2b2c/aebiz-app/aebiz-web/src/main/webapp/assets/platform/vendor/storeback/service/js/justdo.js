...
//			console.log("-------obj.router = " + obj.router);
		...
		getCustomerName: function(customerUuid) {
			...
			$.getJSON(jsContextPath+"/storeback/customer/getCustomerInfoByUuid/"+customerUuid, {ranNum:Math.random()}, function(data) {
				if (data){
		...
		getStoreLogo: function(storeUuid) {
			...
			$.ajaxSettings.async = false; 
			$.getJSON(jsContextPath+"/storeback/storecompanyinfo/getCompanyLogoByUuid/"+storeUuid, {ranNum:Math.random()}, function(data) {
				if (data){
				   var storeInfo = data.storeInfo;
				   var imageUrl = storeInfo.logoPath;
				   if(imageUrl == "" || imageUrl == null){
...
//				alert("暂无新消息！");
		...
		DO_chooseEmoji: function(unified, unicode) {
			...
			var content_emoji = '<img imtype="content_emoji" emoji_value_unicode="' + unicode + '" style="width:18px; height:18px; margin:0 1px 0 1px;" src="/static/storeback/service/img/img-apple-64/' + unified + '.png"/>';
